...
В работе используются образы не confluent, a bitnami, которые поддерживают работу напрямую с PEM-сертификатами без создания JKS/PKCS12, но мы их всё равно создадим для соответствия учебному модулю.
...
keytool -import -file ssl/ca.crt -alias ca -keystore ssl/kafka-1-creds/kafka.kafka-1.truststore.jks -storepass your-password -noprompt
...
keytool -import -file ssl/ca.crt -alias ca -keystore ssl/kafka-2-creds/kafka.kafka-2.truststore.jks -storepass your-password -noprompt
...
keytool -import -file ssl/ca.crt -alias ca -keystore ssl/kafka-3-creds/kafka.kafka-3.truststore.jks -storepass your-password -noprompt
